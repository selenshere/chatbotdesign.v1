...
if (state.name?.firstName && state.name?.lastName && state.preQuestions.q1 && state.preQuestions.q2 && state.preQuestions.q3) {
  showChat();
} else {
  showWelcome();
}
...
function renderChat(){
  ...
  for (const m of state.messages) {
    ...
    if (m.who === "taylor") {
      ...
      // Allow re-opening analysis by clicking any Taylor response.
...
// Save & return: auto-save + close popup + enable chat
if (saveReturnBtn) {
